...
function sendAccountValidationURLToEmail(user: User): Promise<IBaseMessage> {
  return new Promise(function (resolve, reject) {
    ...
    UserAuthenticationToken.create({ tokenType: ACCOUNT_VALIDATION_TOKEN_TYPE })
      ...
      .then((_: any) => {
        return Email.sendEmail({
          ...
          text:
            `Welcome to ${APP_NAME}! Before you can log in, please click ` +
 ...
 */
export function sendAccountValidationLink(
  emailAddress: string
...
export function validateAccount(validationURI: string): Promise<IBaseMessage> {
  return new Promise(function (resolve, reject) {
    UserAuthenticationToken.findOne({
      where: {
      ...
      .then(async (token: UserAuthenticationToken) => {
        ...
        if (!token) {
          ...
          resolve({
            success: false,
            status: 303,
            redirectURL: `/send-validation-email`,
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (!userName || !password || !emailAddress) {
      resolve({
        ...
        status: 200,
        message:
          "At least one of these wasn't provided: userName, password, email",
      });
      return;
    }
    ...
    User
      ...
      .findOne({
        where: {
          [Op.or]: [{ emailAddress: emailAddress }, { userName: userName }],
        },
      })
...
export function authenticateUser(authDetails: {
  ...
  return new Promise(function (resolve, reject) {
    User.findOne({ where: identifierQuery })
      .then(async (user: User) => {
        if (!user) {
          resolve({
            success: false,
        ...
        try {
          ...
          let sessionToken: UserAuthenticationToken = await UserAuthenticationToken.create(
            {
...
export function deleteSessionToken(tokenValue: string): Promise<IBaseMessage> {
  return new Promise(function (resolve, reject) {
    UserAuthenticationToken.destroy({
  ...
  return new Promise(function (resolve, reject) {
    User.findOne({ where: { emailAddress: emailAddress } })
      .then(async (user: User) => {
        ...
        try {
          let passwordResetToken: UserAuthenticationToken = await UserAuthenticationToken.create(
            {
          ...
          return Email.sendEmail({
            ...
            text:
              `To reset your ${APP_NAME} password, ` +
              `click on this link:\n\n${BASE_URL}` +
    ...
    UserAuthenticationToken.findOne({
      ...
      .then((token: UserAuthenticationToken) => {
        if (!token) {
          ...
          Date.now() >
          token.createdAt.getMilliseconds() + 2 * 3600 * 1000
        ) {
      ...
      .then(async (token: UserAuthenticationToken) => {
        ...
        try {
          ...
          return Email.sendEmail({
            ...
            text:
              `Your ${APP_NAME} password was reset on ${resetRequest.timeOfRequest}. ` +
...
export function updateStreak(
  payload: Pick<ISearchQuery, "userId" | "cardIds">
): Promise<IBaseMessage> {
  payload = sanitizeQuery(payload);
  return new Promise(function (resolve, reject) {
    ReviewStreak.findOne({ where: { userId: payload.userId } })
 ...
 */
export function writeCardsToJSONFile(
  userId: string
