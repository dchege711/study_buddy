 ...
 */
async function cardTagsToArray(
  card: IClientFacingFlashCard
): Promise<string[]> {
  try {
    let tagValues: string[];
    (await card.getTags()).forEach((tag) => {
...
export function update(
  incomingCard: IClientFacingFlashCard
): Promise<IBaseMessage> {
  incomingCard = sanitizeCard(incomingCard);
  return new Promise(function (resolve, reject) {
    FlashCard.findByPk(incomingCard.id)
      .then((card: FlashCard) => {
        if (!card) {
 ...
 */
let collectMatchingCards = function (
  queryObject: FindOptions
): Promise<IBaseMessage> {
  return new Promise(function (resolve, reject) {
    FlashCard.findAll(queryObject)
...
export function readPublicCard(
  payload: Pick<ISearchQuery, "cardId">
): Promise<IBaseMessage> {
  payload = sanitizeQuery(payload);
  return new Promise(function (resolve, reject) {
...
export function duplicateCard(
  ...
  return new Promise(function (resolve, reject) {
    ...
    FlashCard.findOne({ where: queryObject })
...
export function flagCard(
  payload: Partial<
    Pick<ISearchQuery, "cardId" | "markedForReview" | "markedAsDuplicate">
  >
): Promise<IBaseMessage> {
  payload = sanitizeQuery(payload);
  ...
  return new Promise(function (resolve, reject) {
    FlashCard
      ...
      .increment(colsToIncrement, {
        by: 1,
        where: { id: payload.cardId, isPublic: true },
      })
      .then((_: any) => {
        resolve({
          status: 200,
...
export function getTagGroupings(
  payload: Pick<ISearchQuery, "userId">
): Promise<IBaseMessage> {
  payload = sanitizeQuery(payload);
  return new Promise(function (resolve, reject) {
    FlashCard.findAll({ where: { ownerId: payload.userId } })
