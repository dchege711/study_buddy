...
export function readPublicCard(req: Request, res: Response) {
  sendResponseFromPromise(
    FlashCardsModel.readPublicCard({ cardId: req.body.cardId }),
    res
  );
}
...
function parsePublicSearchQuery(payload: any): ISearchQuery {
  ...
  if (payload.creationStartDate)
    query.creationStartDate = payload.creationStartDate;
...
export function browsePagePost(req: Request, res: Response) {
  sendResponseFromPromise(
    FlashCardsModel.publicSearch(parsePublicSearchQuery(req.body)),
    res
  );
}
...
export function browsePageGet(req: Request, res: Response) {
  FlashCardsModel.publicSearch(parsePublicSearchQuery(req.query))
    .then((msgContainer) => {
...
export function readTagGroups(req: Request, res: Response) {
  sendResponseFromPromise(
    FlashCardsModel.getTagGroupings({ userId: req.session.user.id }),
    res
  );
}
 ...
 */
function parseClientFacingCard(payload: {
  [s: string]: any;
}): IClientFacingFlashCard {
...
export function downloadUserData(req: Request, res: Response) {
  UserModel.writeCardsToJSONFile(req.session.user.userIDInApp)
...
export function deleteAccount(req: Request, res: Response) {
  UserModel.deleteAccount({ id: req.session.user.id })
    .then((confirmation) => {
...
export function updateUserPreferences(req: Request, res: Response) {
  ...
  UserModel.updateUserPreferences(req.session.user.id, newPrefs)
    .then((confirmation) => {
