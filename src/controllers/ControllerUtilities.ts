...
export function convertObjectToResponse(
  ...
  if (status >= 200 && status < 300) {
    ...
    res.redirect(
      status,
      messageContainer.redirectURL +
        "?msg=" +
        encodeURIComponent(messageContainer.message)
    );
 ...
 */
export function sendResponseFromPromise(
  pendingPromise: Promise<any>,
  res: Response
) {
  pendingPromise
    .then((results) => {
      exports.convertObjectToResponse(null, results, res);
    })
    .catch((err) => {
      exports.convertObjectToResponse(err, null, res);
    });
}
