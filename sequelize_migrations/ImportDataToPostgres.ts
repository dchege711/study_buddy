...
import { User, INewFlashCard, FlashCard } from "../src/models/db/DBModels";
...
async function transferUser(mongoUser: MongoUser): Promise<string> {
  try {
    let preExistingUser = await User.findOne({
    ...
    console.log(
      `Created <${user.userName}>: prefs - ${userPrefs.id}, auth - ${authData.id}`
    );
    return user.id;
  } catch (err) {
    console.error(err);
...
function transferFlashCards(db: Db) {
    ...
    async function (oldCard: MongoCard) {
      try {
        ...
        let card: FlashCard = await (
          await FlashCardsModel.create(<INewFlashCard>newCard)
        ).message;
  ...
  Object.keys(oldParentToOldChildren).forEach(async (oldParentId) => {
    try {
      ...
      console.info(
        `Imported ${children.length} children for '${parent.title}'`
      );
    } catch (err) {
      console.error(err);
    }
...
async function markCardsAsTrashed(oldCardIdToTrashTimestamp: {
  [s: string]: number;
}) {
  try {
    let oldTrashedCardIds = Object.keys(oldCardIdToTrashTimestamp);
    for (let i = 0; i < oldTrashedCardIds.length; i++) {
      let card = await FlashCard.findByPk(
        oldCardIdToNewCardId[oldTrashedCardIds[i]]
      );
      card.trashedTimestamp = new Date(
        oldCardIdToTrashTimestamp[oldTrashedCardIds[i]]
      );
      await card.save();
      console.info(
        `Card [${card.title}] was trashed on ${card.trashedTimestamp}`
      );
    }
  } catch (err) {
    console.error(err);
...
async function transferStreak(metadata: MongoMetadata) {
  try {
    ...
    console.info(
      `[${user.userName}] has a ${streak.streakLength} day streak with ${cardsInStreak.length} cards`
    );
  } catch (err) {
    console.error(err);
  }
