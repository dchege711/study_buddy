...
cards_manager = function(tags_and_ids, user_id) {
    ...
    var cards_to_view = max_PQ();

    /* Hold the IDs of the cards that have already been viewed. */
    var cards_already_viewed = max_PQ();
    ...
    cards_manager_obj.initialize = function(tags_to_use) {
        ...
        for (var tag in tags_to_use) {
            for (var card_id in tags_and_ids[tag]) {
                cards_to_view.insert(
                    [card_id, tags_and_ids[tag][card_id].urgency]
                );
            }
        }

        for (var item in cards_to_view) {
            console.log(item);
        }
    };
     ...
     * @param {Function} call_back The function to call that accepts a card
     * as a parameter.
     */
    cards_manager_obj.next = function(call_back) {
        if (cards_already_viewed.is_empty()) {
            transfer_item(cards_to_view, cards_already_viewed);
        } 
        
        else if (cards_to_view.is_empty()) {
            call_back({});
        }

        var next_card_id = transfer_item(cards_to_view, cards_already_viewed);
        find_card(next_card_id, call_back);
    };
     ...
     * as a parameter.
     */
    cards_manager_obj.previous = function(call_back) {
        if (cards_to_view.is_empty()) {
            transfer_item(cards_already_viewed, cards_to_view);
        }

        else if (cards_already_viewed.is_empty()) {
            call_back({});
        }

        var prev_card_id = transfer_item(cards_already_viewed, cards_to_view);         
    };
     ...
     * @param {Function} call_back The function to be called once the card is
     * found
     */
    function find_card(card_id, call_back) {
        var card = JSON.parse(localStorage.getItem(next_card_id));
        if (card) {
            call_back(card);
        } else {
            sendHTTPRequest("POST", "/read-card", {
                ...
                localStorage.setItem(card_id, JSON.stringify(fetched_card));
                call_back(fetched_card);
            });
        }
    }
...
if (typeof require !== "undefined" && require.main === module) {
    ...
    console.log(test_cards_manager);
