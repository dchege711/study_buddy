...
exports.resetTagSelection = function () {
    tagsState.selectedTags.forEach(function (tag) {
...
exports.initializeTagsBar = function (tagBarElementID, tagsAndIDs) {
    var tagsInDecreasingOrder = Object.keys(tagsAndIDs);
    tagsState.tagsAndIDs = tagsAndIDs;
    tagsInDecreasingOrder.sort(function (tagA, tagB) {
        var importanceTagA = 0;
        var importanceTagB = 0;
        Object.keys(tagsAndIDs[tagB]).forEach(function (cardID) {
            importanceTagB += tagsAndIDs[tagB][cardID].urgency;
            importanceTagB += 1;
        });
        Object.keys(tagsAndIDs[tagA]).forEach(function (cardID) {
    ...
    var tagsHTML = "";
    tagsInDecreasingOrder.forEach(function (tag) {
        var tag_prefix = "#";
        if (tag.charAt(0) == "#")
            tag_prefix = "";
...
exports.getSelectedTags = function () {
    if (tagsState.selectedTags.size == 0)
        return null;
...
exports.getIDsOfSelectedTags = function () {
    if (tagsState.selectedTags.size == 0)
        return null;
    var setCardIDs = new Set([]);
    tagsState.selectedTags.forEach(function (tag) {
        for (var cardID in tagsState.tagsAndIDs[tag]) {
