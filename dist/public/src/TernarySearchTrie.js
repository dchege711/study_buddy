...
function TernarySearchTrie(words) {
    ...
    function getKey(x, key, i) {
        ...
        var c = key.charAt(i);
        if (c < x.c)
            return getKey(x.left, key, i);
        else if (c > x.c)
            return getKey(x.right, key, i);
        else if (i < key.length - 1)
            return getKey(x.mid, key, i + 1);
        else
            return x;
    }
    /**
    ...
    function insertKey(x, key, i) {
        ...
        if (current_char < x.c)
            x.left = insertKey(x.left, key, i);
        else if (current_char > x.c)
            x.right = insertKey(x.right, key, i);
        else if (i < key.length - 1)
            x.mid = insertKey(x.mid, key, i + 1);
        else
            x.val = true;
        return x;
    }
...
if (typeof require !== "undefined" && require.main === module) {
    ...
    for (var i = 0; i < words_two.length; i++)
        testTST.put(words_two[i]);
