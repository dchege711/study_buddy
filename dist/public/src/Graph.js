"uses strict";
var MaxPQ = require("./MaxPriorityQueue.js");
/**
...
function Graph(directed) {
     ...
     *
     */
    this.addEdge = function (nodeOneID, nodeTwoID, weight) {
        if (weight === void 0) { weight = 1; }
        if (Graph.__adj[nodeOneID] === undefined)
            Graph.__adj[nodeOneID] = {};
        Graph.__adj[nodeOneID][nodeTwoID] = weight;
        if (!directed) {
            if (Graph.__adj[nodeTwoID] === undefined)
                Graph.__adj[nodeTwoID] = {};
     ...
     */
    this.kNearNeighbors = function (nodeIDs, k) {
        if (k === void 0) { k = 7; }
        var kNeighbors = new Map([]);
        var alreadySeenIDs = new Set(nodeIDs);
        var pq = new MaxPQ();
        function __enqueue(node, currentPQ) {
            if (!Graph.__adj[node])
                return;
            Object.keys(Graph.__adj[node]).forEach(function (otherNode) {
                if (!alreadySeenIDs.has(otherNode)) {
                    currentPQ.insert([otherNode, Graph.__adj[node][otherNode] * -1]);
                }
            });
        }
        nodeIDs.forEach(function (nodeID) { __enqueue(nodeID, pq); });
        var nodeAndNegatedDistance, nextPQ;
